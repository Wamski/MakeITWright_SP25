...
public class LoginScreen extends JFrame implements FocusListener{
    ...
    public LoginScreen() {
        ...
        robotPanel.setLayout(new FlowLayout(FlowLayout.CENTER, 10, 0));
    
        JCheckBox robotBox = new JCheckBox();
        robotPanel.add(robotBox);

        JLabel robotLabel = new JLabel("I am not a robot ");
        robotLabel.setBackground(blue3);
        ...
        LoginButton.addActionListener(e -> {
            for(CredentialsObj credentialsObj : RegisterScreen.getCredentials()) {
                ...
                if (usernameTextField.getText().equals(credentialsObj.getUsername()) && passwordTextField.getText().equals(credentialsObj.getPassword()) && robotBox.isSelected()){
                    new OpenDeleteSet("Open");
                    this.dispose();
                }
            }
            if(!robotBox.isSelected()){
                incorrectLabel.setText("Please select I am not a robot");
